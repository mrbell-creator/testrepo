...
if not data or len(data) < 8:  # Minimal valid length check  
    return None  
      
try:  
    header = data[:2]  # BLE advertisement header (e.g., "1AFF")  
    mfrData = data[2:]  
    ...
    # --- Advertisement (Peak) Data Parsing ---  
    adv = []  
    last_time = 0  
    w = 6  # starting index for advertisement data  
    data_length = len(mfrData)  
      
    # Extract up to 12 peaks (each encoded in 10 bits)  
    for q in range(12):  
        bitpos = q * 10  
        bytepos = bitpos // 8  
        off = bitpos % 8  
        ...
        v = v >> off  
        dt = (v & 0x1F) + 1  # delta time (dt) minus 1, then +1  
        v = v >> 5  
        amp = v & 0x1F       # raw amplitude  
...
Uses the reference voltage (vref) and the time value.
"""
c_val = 0.5 * vref
...
peak_data: list of dicts with keys:  
          - "a": amplitude  
          - "i": time offset (i = this_time * 2)  
Returns a calculated time-of-flight (TOF) value.  
"""  
if not peak_data:  
    return 0  
c = peak_data  
...
# Build segmentation index list k.  
k = [0]  
for b in range(1, e):  
...
# For each segment, find the index with maximum amplitude.  
g_list = []  
p = 0  
num_segments = len(k)  
...
# Second accumulation loop using segments defined by g_list.  
num_segments = len(g_list)  
for b in range(num_segments):  
    ...
    if idx < 100:  
        m_arr[idx] += m_adj  
    for f in range(b+1, num_segments):  
...
"""
Legacy conversion from TOF to level in inches.
Previously the JS code used: 15000 * tof + 0.5.
We now use 13700 as a tweak:
level (inches) = 13700 * tof + 0.5
"""
return 13700 * tof + 0.5

def legacy_level_cm(tof):
"""Converts legacy level from inches to centimeters (1 inch = 2.54 cm)."""
return legacy_level_inches(tof) * 2.54

def legacy_level_percentage(tof, tank_height=0.254):
"""
Computes the percentage full based on the legacy level.

tank_height: The full tank height in meters (default is 0.254 m for a 20‑lb tank).  
The legacy algorithm is:  
  If tank_height <= 0.0381 m, return 100%.  
  Otherwise, let:  
      level_meters = (legacy_level_cm(tof) / 100)  
      percentage = 98 * (level_meters - 0.0381) / (tank_height - 0.0381) + 2  
  Clamp the result to be between 2 and 100, then round.  
"""  
if tank_height <= 0.0381:  
    return 100  
level_meters = legacy_level_cm(tof) / 100.0  # convert cm to meters  
percentage = 98 * (level_meters - 0.0381) / (tank_height - 0.0381) + 2  
if percentage < 2:  
    percentage = 2  
if percentage > 100:  
...
# Add the computed values to the parsed data  
result = parsed.copy()  
result["tof"] = tof  
...
"1AFF0D000002AC67153CE04100000C30C000036C9080074D287B270302A0AD",  # Empty tank
]

Is this the code we need to change to also account for xl?
