...
def hex_to_bytearray(hex_string):
    ...
    try:
        ...
        print(f"[hex_to_bytearray] Invalid hex string: {hex_string}")
        return None

def parse_payload(hex_payload):
    """Parses a Mopeka BLE advertisement payload into structured sensor data."""
    data = hex_to_bytearray(hex_payload)
    if not data or len(data) < 8:
        print(f"[parse_payload] Invalid or too short data: {hex_payload}")
        return None
    ...
    try:
        ...
        if hw_family == "xl":
            print("[parse_payload] Detected XL sensor.")
            last_time = 0
            w = 6
            ...
            print("[parse_payload] Detected Gen2 sensor.")
            i = 6
        ...
        print(f"[parse_payload] Parsed peaks: {adv}")
...
def compute_pulse_echo_time2(peak_data, vref=1.2421875):
    ...
    for b in range(e):
        ...
        for f in range(b + 1, e):
            ...
            r_val = c[f]['a']
            r_adj = min(adjust_amplitude(r_val, q_val, vref), p_adj)
    ...
    g_max = max((peak['a'] for peak in c), default=0)
    if g_max <= 0.63453125:
        return 0

    tof = 2e-5 * n_index
    print(f"[compute_pulse_echo_time2] TOF: {tof:.8f}")
    return tof
...
def legacy_level_percentage(tof, tank_height=0.254):
    ...
    return max(2, min(100, round(percentage)))
...
def process_hex_string(hex_string, tank_height=0.254):
    ...
    result = parsed.copy()
    result["tof"] = tof
    result["level_inches"] = legacy_level_inches(tof)
    result["level_cm"] = legacy_level_cm(tof)
    result["is_empty"] = is_empty
    result["percentage"] = 0 if is_empty else legacy_level_percentage(tof, tank_height)

    print(f"[process_hex_string] Final output: {result}")
    return result
